...
(define (draw-spline cr win)
  (define (sp n)
    (vector-ref (spline-of win) n))
  (let ((zoom   (zoom-of win))
        (drag-user-x (x-of (drag-pt-of win)))
        (drag-user-y (y-of (drag-pt-of win))))
    (cairo-device-to-user cr drag-user-x drag-user-y)
    ;;(format #t "drag-user(x,y)=(~a,~a)~%" drag-user-x drag-user-y)
    (cairo_save cr)
    (cairo_move_to cr  (x-of (sp 0)) (y-of (sp 0)))
    (cairo_curve_to cr 
                    (x-of (sp 1)) (y-of (sp 1))
                    (x-of (sp 2)) (y-of (sp 2))
                    (x-of (sp 3)) (y-of (sp 3)))
    ...
    (if (and (click-of win)
             (nz? (cairo_in_stroke cr 
                                   (/. drag-user-x zoom) ; XXX
                                   (/. drag-user-y zoom))))
        (set! (active-of win) #xf))

    (cairo_stroke cr)

    (draw-control-line cr (sp 0) (sp 1) (/ 2.0 zoom))
    (draw-control-line cr (sp 3) (sp 2) (/ 2.0 zoom))
...
(define (win-handle-button-press win bev)
  ;;(format #t "bev(x,y)=(~a,~a)~%" (ref bev 'x) (ref bev 'y))
  (set! (click-of win) #t)
...
(define (print-spline win)
  (define (sp n) (vector-ref (spline-of win) n))
